...
# ╔═╡ b4b71589-db0f-463b-9e87-bda7bacfa2ca
begin
	import Pkg
	Pkg.activate(".")
end
...
# ╔═╡ a2a1f228-7b76-4a4e-8907-a8a8bfe06300
md"""#### Equation:  Y = X $\times$ $(@bind weight Scrubbable(range(-1, 3.0, step=0.25), default=0)) + $(@bind c Scrubbable(-5:5; default = 0))

Y = X  $\times$ p + constant"""

# ╔═╡ c14bf31e-f98c-11ea-33d1-f5de8f2ad0c1
md"""
make it more realistic by using **noisy data?** $(@bind show_noisy CheckBox())
"""
...
# ╔═╡ d77645b1-75e0-468a-88e3-c246b6bc0388
md"## ... in action"
...
addNoise = 0.7 .* randn(nPoints);

# ╔═╡ 8029070e-efb0-11ea-1d0a-7f402f2e9989
begin
	t = 0:1:(nPoints - 1);
	y_data = 1.5.*t;
	y_data_noise = y_data + addNoise;
	md"setting some variables..."
end
...
begin
	...
	plot(t,y, lw=2, color=:red,
		aspect_ratio=1, 
		xlims=(0, 15), ylims=(-3, 15),
		xticks=0:2:14, yticks=0:2:14,
		label="fit curve", framestyle=:origin, legend=:right)
	if show_noisy
		y = y_data_noise
	else
		y = y_data
	end
	plot!(t,y, seriestype=:scatter, marker=:square, ms=5, color=:black, label=:"data points")

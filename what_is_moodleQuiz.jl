...
**x:** $(@bind xs Slider(1:size(fmri_data,1), default=26, show_value=true)), 
**y:** $(@bind ys Slider(1:size(fmri_data,2), default=10, show_value=true)), 
**z:** $(@bind zs Slider(1:size(fmri_data,3), default=3, show_value=true))
...
begin
	p1_ = heatmap(view(fmri_data, :, :, zs, 1), 
		aspect_ratio=1,
		c=:greys, colorbar=:none);
	xaxis!(showaxis=:no);
	plot!([ys], [xs], m=:square,
		msw=4, msc=:red, ms=4, c=:red, legend=false);
	plot!(size=(300,300));
	"image slice code..."
end
...
begin
	...
	plot(p, p1_, layout = grid(1, 2; widths = [0.8 ,0.2]),)
end
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "a7ed7b57edc39c949d962f3ac92d9b7f9c7d08a5"
